...
// Provenance (PB-PIN-04): the summary is committed and published later, so the page must say
// when and from what it was measured. run-perf.mjs supplies these.
const GENERATED_AT = __ENV.PERF_GENERATED_AT || '';
const SOURCE_REF = __ENV.PERF_SOURCE_REF || '';
const RUN_URL = __ENV.PERF_RUN_URL || '';
...
export function handleSummary(data: SummaryData): Record<string, string> {
  return {
    ...
    'perf-summary.json': JSON.stringify({ ...data, provenance: provenance() }, null, 2),
...
<p class="meta"><strong>Measured ${esc(GENERATED_AT || 'date not recorded')}</strong>${
    SOURCE_REF ? ` from commit <code>${esc(SOURCE_REF.slice(0, 7))}</code>` : ''
  }${RUN_URL ? ` · <a href="${esc(RUN_URL)}">workflow run</a>` : ''}.
These figures are a snapshot from that run, not a live measurement.</p>
