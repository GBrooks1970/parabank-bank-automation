...
The following official stable releases were reviewed and resolved on 2026-08-01:
...
The workflow grants only `contents: read`. A future action that requires another token
permission must justify the narrowest additional permission in the same PR. Write access
must never be added merely to silence an action failure.
